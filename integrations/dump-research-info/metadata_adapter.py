...
def load_yaml_records(downstream_root: Path) -> dict[str, dict[str, object]]:
    ...
    for scope, relative_root in (
        ...
        for path in sorted(root.rglob("*.yaml")):
            ...
            if pid in result:
                raise DumpResearchInfoAdapterError(
                    f"Downstream PID is duplicated: {pid}"
                )
            result[pid] = {
...
def render_markdown(report: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        "This is source-review evidence, not an approved canonical metadata update.",
        "",
        "## Summary",
        "",
    ...
    lines.extend(
        [
            ...
            "copying any value into canonical downstream metadata.",
            "",
        ]
    )
    return "\n".join(lines)
...
def extract(
    ...
    *,
    ...
    metadata_dirty = paths_differ_from_commit(
        ...
        ["metadata/records", "metadata/reference"],
    )
    report: dict[str, Any] = {
...
def review(context: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        "adapter_api_version": PLUGIN_API_VERSION,
        ...
        "blockers": (
            ...
            if context.get("mode") == "review"
                ...
                "dump-research-info evidence must be generated by the "
                "datalad-run-dump-research-info task"
            ]
        ),
        "warnings": report["warnings"],
        "summary": report["summary"],
