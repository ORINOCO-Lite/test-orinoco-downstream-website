...
This site-owned integration is the first proof of the Orinoco metadata-adapter goal.
...
This keeps executables and symlinks out of the site-owned `integrations/` tree that Orinoco validates and packages.
...
[`evidence/con-site-gap/`](evidence/con-site-gap/) is a separate DataLad-run comparison of the historical `dump-research-info:data/con_site` extraction with the current downstream metadata.
It is retained as review evidence and a possible input to a second adapter; it is not loaded by this host or the site.
