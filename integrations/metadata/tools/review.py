...
def load_config(path: Path = CONFIG) -> list[dict[str, object]]:
    ...
    if document.get("contract_version") != PLUGIN_API_VERSION:
...
def load_plugin(root: Path, source: Mapping[str, object]) -> ModuleType:
    source_id = str(source["id"])
    path = safe_repo_path(root, source["plugin"], label=f"{source_id} plugin")
    if not path.is_file() or path.is_symlink():
        raise MetadataReviewError(f"Adapter plugin is not an ordinary file: {path}")
    name = f"orinoco_metadata_adapter_{source_id}"
    specification = importlib.util.spec_from_file_location(name, path)
    if specification is None or specification.loader is None:
        raise MetadataReviewError(f"Cannot load adapter plugin {path}")
...
def validate_result(
    ...
    if result.get("adapter_api_version") != PLUGIN_API_VERSION:
...
def run(
    ...
    *,
    ...
    for source in active_sources:
        ...
        output.mkdir()
        module = load_plugin(root, source)
        context = {
            "mode": mode,
            "root": str(root),
            "output": str(output),
