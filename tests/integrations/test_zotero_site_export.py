...
SCRIPTS = ROOT / "integrations/zotero/tools"
...
class ZoteroSiteExportTests(unittest.TestCase):
    ...
    def export(
        ...
        *,
        ...
        EXPORT.command_export(
            argparse.Namespace(
                publications=(
                    ROOT / "integrations/zotero/source/candidates/XYZPublication.json"
                ),
                snapshot=(
                    ROOT / "integrations/zotero/source/snapshot.json"
                ),
                policy=policy
                or ROOT
                / "integrations/zotero/policy/site-migration.yaml",
                output_dir=output,
                report=report,
            ),
    ...
    def test_export_fails_closed_on_an_unreviewed_relationship_target(self) -> None:
        policy = EXPORT.load_policy(
            ROOT / "integrations/zotero/policy/site-migration.yaml"
        )
        record = {
    ...
    def test_export_rejects_an_input_inside_the_output_artifact(self) -> None:
        with self.build_temporary_directory() as directory:
            ...
            output = root / EXPORT.OUTPUT_RELATIVE_PATH
            output.mkdir()
            policy = output / "site-migration.yaml"
            policy.write_bytes(
                (
                    ROOT
                    / "integrations/zotero/policy/site-migration.yaml"
